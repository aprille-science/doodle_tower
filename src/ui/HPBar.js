export default class HPBar {
  constructor(scene, x, y, width, height, color, label, opts = {}) {
    this.scene = scene;
    this.x = x;
    this.y = y;
    this.width = width;
    ...
    this.label = scene.add.text(x, y - 16, label, {
      fontSize: '12px', color: this.labelColor, fontFamily: 'monospace'
    });
    this.valueText = scene.add.text(x + width + 8, y + 2, '', {
      fontSize: '11px', color: this.labelColor, fontFamily: 'monospace'
    });
  ...
  update(current, max) {
    ...
    this.draw(ratio);
    this.valueText.setText(`${Math.ceil(current)}/${max}`);
  }
